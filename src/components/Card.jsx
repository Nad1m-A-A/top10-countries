...
function Card(props) {
    ...
    const numbersFiller = CTX.aspectName === 'wealthy' ? 'trillion' : CTX.aspectName === 'poor' ? 'billion' : CTX.aspectName === 'at_war' ? 'thousands' : '';
    const dollarSign = CTX.aspectName === 'wealthy' || CTX.aspectName === 'poor' ? '$' : '';

    return (
        <>
            <div className='card card-fade-in'>
                <div className="order">{props.index} - {props.name}</div>
                <div className="flag">
                <img src={props.flag} alt="country flag"/>
                </div>
                <span>Continent: {props.continents}</span>
                <span>Language: {Object.values(props.languages)[0]}</span>
                {<span>{stringSorter()}: {dollarSign}{props.numbers} {numbersFiller}</span>}
            </div>
        </>
    )
}
